...
| `codex-bridge` (opt-in, `CODEX=on`) | external CLI — gpt-5.5 (`~/.codex/config.toml`) | Blind second opinion / offloaded self-contained clear-spec bulk via `codex exec`; its dispatcher-attested deadline is enforced by the contained Python runner, never a shell timeout helper. |
