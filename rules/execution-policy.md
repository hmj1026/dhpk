...
The CLI-backed Codex roles use the same normalized project-over-global configuration seam for timeout budgets: `codex_fast_worker_timeout_secs`, `codex_deep_reasoner_timeout_secs`, and `codex_bridge_timeout_secs` override shared `codex_timeout_secs` within their scope, with the shipped `360`-second default below both scopes. `0` is an intentional dispatcher-attested no-deadline request; malformed values fail closed before the affected dispatch. The portable runner reports the effective role, budget, source, disabled state, and explicit outer-budget unknown/warning status without changing Claude's external tool wait; it never falls back to a shell timeout tool.
