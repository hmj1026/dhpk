...
| `review_agents` | `["code-reviewer","database-reviewer","security-reviewer","frontend-reviewer","doc-reviewer","polyfill-reviewer","migration-reviewer","artifact-reviewer"]` | Eight agents invoked by sentinel reminders (slots: code, db, sec, frontend, doc, polyfill, migration, artifact). Override to point at your project-specific agents; shorter overrides are padded with the defaults for the remaining slots. (Slots 5–6 fire only when opted in — polyfill via `library-author`, migration via module triggers or a `mig:` extra path; slot 7 artifact is always-on for any `.md` with YAML frontmatter.) |
...
```bash
# Plain install with defaults (8-slot reviewer dispatch on the default agent names).
claude plugin install dhpk@dhpk
...
├── agents/                       # 25 role-based agents (INDEX.md is navigation)
