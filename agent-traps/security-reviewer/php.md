...
Generic PHP OWASP examples (injection, XSS, CSRF, file upload, deserialization) live at `skills/dhpk-php-runtime-router/references/agent-extracts/security-owasp-examples.md`. Yii 1.1-specific CSRF, ownership, and encoding live in `security-reviewer/yii.md`.
...
| `unserialize($userData)` on untrusted input (PHP object injection) | `json_decode($userData, true)` + schema validation | `unserialize()` on framework session |
...
| Weak password crypto — `md5()` / `sha1()` for passwords | `password_hash()` / `password_verify()` (bcrypt/argon2) | `md5()` for cache key / filename |
