...
- Authoring or refactoring a skill — use `dhpk-create-skill` (it calls this skill first).
- Auditing installed skills for quality, duplicates, or staleness — use
  `dhpk-skill-health-audit` (lint) or `dhpk-skill-stocktake` (batch audit).
...
> Entries marked `(not in <your-project>)` are not yet ported to this project. References kept for upstream parity.

- `search-first` - General search-before-building workflow. `(not in <your-project>)`
- `dhpk-skill-stocktake` - Audit installed skills for health, duplicates, and gaps.
- `agent-sort` - Categorize and organize existing agents and skills. `(not in <your-project>)`
