...
ANALYSIS_COOLDOWN="${CLV2_OBSERVER_ANALYSIS_COOLDOWN:-60}"
IDLE_TIMEOUT_SECONDS="${CLV2_OBSERVER_IDLE_TIMEOUT_SECONDS:-1800}"
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${CLV2_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping claude analysis on Windows due to known non-interactive hang issue (#295). Set CLV2_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi
  ...
  MAX_ANALYSIS_LINES="${CLV2_OBSERVER_MAX_ANALYSIS_LINES:-500}"
  observer_tmp_dir="${PROJECT_DIR}/.observer-tmp"
  mkdir -p "$observer_tmp_dir"
  analysis_file="$(mktemp "${observer_tmp_dir}/clv2-observer-analysis.XXXXXX.jsonl")"
  ...
  prompt_file="$(mktemp "${observer_tmp_dir}/clv2-observer-prompt.XXXXXX")"
...
PROMPT
  ...
  timeout_seconds="${CLV2_OBSERVER_TIMEOUT_SECONDS:-120}"
  max_turns="${CLV2_OBSERVER_MAX_TURNS:-20}"
  exit_code=0
  ...
  CLV2_SKIP_OBSERVE=1 CLV2_HOOK_PROFILE=minimal claude --model haiku --max-turns "$max_turns" --print \
