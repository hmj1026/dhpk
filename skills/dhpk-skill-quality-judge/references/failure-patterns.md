...
Self-contained catalog for `dhpk-skill-quality-judge`. Load this when you have a low score and need to name the failure mode precisely, when you want the evaluator's own "never do" guardrails, or when running the exhaustive quick-check before finalizing a report.
