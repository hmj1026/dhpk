...
| Feature Delivery / Bug Investigation & Fix | 機械式、規格明確 | 共用 selector 解出的 `dhpk:fast-worker` / `dhpk:codex-fast-worker` / `dhpk:agy-fast-worker` |
...
`@rules/execution-policy.md` → *Post-implementation agent gate (SSOT)*。依本次變更分類標示需要的
implementation specialist 與 sentinel reviewer；所有適用 reviewer 以每個 implementation wave 一批
consolidated parallel batch 執行，不把 `tdd-guide` 或 `e2e-runner` 當成無條件 post-edit gate。

**Gate 失敗處理：** findings 合併成一份 fix-spec；整批超過 inline bound 時交給 selector-resolved
fast worker。已知 findings 只做一次 confirm-only 複查；TDD/E2E 修正回到原 specialist 的驗證命令。
