...
<!-- SSOT: @rules/execution-policy.md Agent 調用清單（dhpk plugin 出貨；專案可加 .claude/rules/execution-policy.md delta）。本節為輸出 checklist 用途，若兩者衝突以 execution-policy.md 為準。-->

回覆中**必須**輸出以下 checklist，根據 workflow 分類預填 YES/NO：

實作完成後，依序執行以下 agents（`@rules/execution-policy.md` 強制後置步驟）：

| 順序 | Agent | 觸發條件 | 此次適用? |
|------|-------|---------|---------|
| 1 | dhpk:tdd-guide | bug fix 或新功能 | [YES/NO] |
| 2 | dhpk:database-reviewer | 涉及 SQL / 資料庫 | [YES/NO] |
| 3 | dhpk:security-reviewer | 涉及認證/授權/加密/金額 | [YES/NO] |
| 4 | dhpk:code-reviewer | 無例外，最後一步 | YES |

根據變更描述的上下文預填 YES/NO。

**Gate 失敗處理：**
若任一 gate 失敗 → 修正問題 → 重新執行該 gate → 直到 PASS 後繼續下一 gate。不得跳過。
...
| Feature Delivery（跨模組） | dhpk:architect | `/opsx:new` | profile + work-item + legacy-ref + RED |
| Feature Delivery（一般） | — | `/opsx:new` | profile + work-item + legacy-ref + RED |
...
**Feature Delivery:**
```
Requirements → OpenSpec → TDD → Implement → Review
                  │          │       │          │
                  ▼          ▼       ▼          ▼
             /opsx:new  tdd-guide  Edit   db+sec+code
                                          reviewers
```
