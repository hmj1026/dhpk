#!/usr/bin/env bash
# Portable asset adapter for the relocated harness-setup Skill.
# The source artifact is data; only the synchronized writer beside this file
# is executable.
set -u

ADAPTER_NAME="harness-setup/install-assets"
...
usage() {
    cat <<'EOF'
Usage: install-assets.sh --source-artifact DIR --target DIR --install hooks|rules|scripts|all [--dry-run] [--force]
...
stdout_file="$(mktemp "${TMPDIR:-/tmp}/dhpk-setup-assets.XXXXXX")" ||
    fail_result WRITER_FAILED 'cannot allocate writer output buffer'
stderr_file="$(mktemp "${TMPDIR:-/tmp}/dhpk-setup-assets.XXXXXX")" || {
    rm -f "$stdout_file"
