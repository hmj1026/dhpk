...
the conflict before continuing. Defer to `dhpk-laravel-testbench-matrix` skill
...
This step deliberately overlaps with the `dhpk-polyfill-version-matrix-audit`
...
- **Floor-raising disguised as cell add**: if adding the new cell requires
  ...
  `dhpk-composer-package-hygiene` skill for the decision tree.
...
- `dhpk-laravel-testbench-matrix` — per-cell Testbench install + DB config
- `dhpk-composer-package-hygiene` — semver decision when raising the floor
- `dhpk-polyfill-version-matrix-audit` — deep audit when step 4 surfaces gaps
