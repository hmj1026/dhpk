...
- Bug investigation (use `dhpk-bug-fix` or `dhpk-issue-analyze`)
- Architecture overview (use `dhpk-codebase-exploration`)
...
```
Input: /dhpk:dhpk-codex-explain src/service/order/order.service.ts
Action: Read file → Codex explain (normal) → Functional summary + detailed explanation

Input: /dhpk:dhpk-codex-explain src/service/xxx.ts --depth deep
Action: Read file → Codex explain (deep) → Patterns + complexity + issues

Input: /dhpk:dhpk-codex-explain src/xxx.ts --lines 50-100
