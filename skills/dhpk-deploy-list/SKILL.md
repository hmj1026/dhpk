...
1. Copy the whole `skills/dhpk-deploy-list/` directory + the matching
   `/dhpk:dhpk-deploy-list` slash command into the project's `.claude/`.
...
bash skills/dhpk-deploy-list/scripts/check-golden.sh

# Update goldens after an intentional schema change
bash skills/dhpk-deploy-list/scripts/check-golden.sh --update
```
