...
5. **Lifecycle clearance is never approval**: exactly like the native path, existence + freshness gate the CLEAR, never verdict-parseability — a fresh malformed, BLOCK, FAIL, or nonzero critical/high/medium doc still reconciles the sentinel lifecycle. Unlike the native path, a resumed reconcile ALSO treats an unparseable verdict as unresolved (not just BLOCK/FAIL/severity>0), so a malformed resumed result can never read as silent approval; `.unresolved-verdict` is refreshed identically to `subagent-stop-verify.sh`'s contract either way.
