---
name: dhpk-python-static-checks
...
Not for writing application logic (see `dhpk-python-pro`), and not for choosing *which*
tests to write (see `dhpk-pytest-async`). Load this only when configuring or triaging
ruff / pyright / mypy.
