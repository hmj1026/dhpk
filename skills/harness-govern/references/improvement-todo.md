# Harness Govern Sync 改進待辦（Run Retrospective）

本文件整理本次執行 `harness-govern sync` 遇到的實際問題，作為 skill 優化待辦。
...
| MAI-007 | P2 | Gate 常見 `PARTIAL`（hooks / multi-agent skip-incompatible）容易被誤判為失敗 | 在 SKILL 與報告中明確定義 `PASS/PARTIAL/FAIL` 決策準則 | 判讀一致性 | DONE |
