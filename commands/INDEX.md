...
| `/codex-review` | Consolidated second opinion: `--scope diff|branch|doc|security|tests`, `--depth fast|full` |
| `/codex-cli-review` | Codex CLI 全專案 review |
| `/codex-explain` | 複雜邏輯解釋 |
| `/codex-implement` | Codex 寫程式 |

> `/codex-review --scope diff --depth fast` 適合快速 diff；`--depth full` 用於交付前；`--scope branch|doc|security|tests` 選擇其他受支援範圍。獨立 security/doc/test 審查仍優先使用專門 reviewer/skill。
...
| Command | 用途 |
|---------|------|
| `/precommit` | Pre-commit 檢查；`--fast` 選擇快速模式 |
...
| `/dhpk:setup` | 設定 plugin；`--install hooks|rules|scripts|all` 安裝目前資產 |
