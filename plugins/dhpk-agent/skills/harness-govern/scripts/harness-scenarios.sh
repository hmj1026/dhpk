...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        --dir)
            [[ -n "${2:-}" ]] || { echo "[error] --dir requires an argument." >&2; exit 2; }
            HARNESS_DIR="$2"; shift ;;
        --execute-hooks) EXECUTE_HOOKS=1 ;;
...
if [[ "$EXECUTE_HOOKS" != "1" ]]; then
    echo "NOT_RUN: project-local hooks require separately approved --execute-hooks." >&2
    exit 3
fi
...
done < <(find "$HARNESS_DIR/hooks" -maxdepth 1 -name '*.sh' -type f 2>/dev/null | sort)
