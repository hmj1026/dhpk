...
Slash command (`/dhpk:dhpk-skill-stocktake`) that audits all Claude skills and commands using a quality checklist + AI holistic judgment. Supports two modes: Quick Scan for recently changed skills, and Full Stocktake for a complete review.

## When NOT to Use

- Linting a single skill's structure / frontmatter — use `dhpk-skill-health-audit`.
- Authoring or refactoring a skill — use `dhpk-create-skill`.
- Searching for an existing skill to adopt before building — use `dhpk-skill-scout`.
- Canonical `./skills` and `./modules` source audits — use `dhpk-skill-health-audit` with explicit directories plus a source-tree inventory; this skill scans consumer `.claude/skills/` locations only.
...
| Full Stocktake | `results.json` absent, or `/dhpk:dhpk-skill-stocktake full` | 20–30 min |

**Results cache:** `~/.claude/skills/dhpk-skill-stocktake/results.json`

## Quick Scan Flow

Re-evaluate only skills that have changed since the last run (5–10 min).

1. Read `~/.claude/skills/dhpk-skill-stocktake/results.json`
2. Run: `bash ~/.claude/skills/dhpk-skill-stocktake/scripts/quick-diff.sh \
         ~/.claude/skills/dhpk-skill-stocktake/results.json`
   (Project dir is auto-detected from `$PWD/.claude/skills`; pass it explicitly only if needed)
3. If output is `[]`: report "No changes since last run." and stop
4. Re-evaluate only those changed files using the same Phase 2 criteria
5. Carry forward unchanged skills from previous results
6. Output only the diff
7. Run: `bash ~/.claude/skills/dhpk-skill-stocktake/scripts/save-results.sh \
         ~/.claude/skills/dhpk-skill-stocktake/results.json <<< "$EVAL_RESULTS"`

## Full Stocktake Flow

### Phase 1 — Inventory

Run: `bash ~/.claude/skills/dhpk-skill-stocktake/scripts/scan.sh`
