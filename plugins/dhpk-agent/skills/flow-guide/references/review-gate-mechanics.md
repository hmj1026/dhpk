...
### Identity-bound evidence presentation

The orchestrator is the owner of dispatch and handoff identity; the reviewer is the producer of the artifact; and the runtime hook/reconcile path is the owner of Sentinel clearance. Bind each handoff and retry to one stable `task_id` plus an `attempt_id` for that attempt, and retain the producer, wave, evidence scope, adapter, and verification stage. `plan_fingerprint` and `artifact_fingerprint` are optional additions. Legacy scope/diff-only artifacts remain valid on the compatibility path, but when a new obligation declares identity, missing or foreign identity fails closed.

Evidence is presented to the existing clearance path as `handoff identity -> fresh artifact marker -> lifecycle verdict -> matching Sentinel`. A passing message, terminal lifecycle state, or aggregate `EvidenceResult` is not a review artifact and cannot clear a Sentinel. The identity envelope must not contain or substitute for the aggregate verdict; the hook evaluates the canonical artifact and its existing verdict/severity rules.
...
1. **Record before resuming**: `bash scripts/hooks/record-resumed-obligation.sh <sentinel-name> [label]` — captures this session's identity (`CLAUDE_CODE_SESSION_ID`), the resolved agent, and an **artifact baseline** (the latest matching canonical review doc, if any, BEFORE the resume) into the `.resumed-review-obligations` JSONL sidecar. The record binds the generated slot, exact sentinel basename, resolved agent identity, origin session, dispatch/resume identity, resume timestamp, artifact baseline, stable task identity, attempt identity, and state. New records may also carry producer, wave, scope, adapter/stage, and optional plan/artifact fingerprints. The command prints a `RESUMED_REVIEW_IDENTITY task_id=... attempt_id=... producer=... wave=... scope_id=... adapter=... stage=...` envelope; when declared, it also includes non-empty `plan_fingerprint=...` and/or `artifact_fingerprint=...` pairs. Copy that exact envelope into the `SendMessage` instruction and require the reviewer to reproduce every declared identity field in the new artifact's frontmatter alongside the canonical `scope: [paths]` field. A fresh artifact missing or changing a declared field stays armed. One active record per (sentinel, session); a repeated resume for the same slot updates the existing record's `attempt` rather than creating a second one. This is lifecycle state, not a new reviewer sentinel slot or active marker. Legacy records without optional identity fields keep the established scope/freshness behavior.
