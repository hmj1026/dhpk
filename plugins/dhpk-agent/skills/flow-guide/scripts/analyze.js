...
function loadClassification() {
  try {
    const p = resolveRuntimeFile('file-classification.json');
...
function runHeuristics(inputs, files, gates, root, featureCtx) {
  ...
  if (hasChanges) {
    ...
    if (reviewState && gates.code_review.required && !gates.code_review.passed) {
      findings.push({
        ...
        suggestion: 'Run /dhpk:change-verdict --mode code before proceeding',
      });
    }
    ...
    if (reviewState && gates.doc_review.required && !gates.doc_review.passed) {
      findings.push({
        ...
        suggestion: 'Run /dhpk:change-verdict --mode docs before proceeding',
      });
    }
  ...
  if (securityFiles.length > 0) {
    findings.push({
      ...
      suggestion: 'Run /dhpk:change-verdict --mode security before merging',
    });
  }
  ...
  if (featureCtx && featureCtx.key) {
    ...
    if (allGatesPass && !hasBlockers && reviewState && gates.precommit.passed) {
      findings.push({
        ...
        suggestion: 'Ready for commit and $change-verdict --mode pr',
      });
    }
  }
