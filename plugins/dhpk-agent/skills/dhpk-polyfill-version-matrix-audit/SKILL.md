---
name: dhpk-polyfill-version-matrix-audit
...
- `dhpk-composer-package-hygiene` — semver decisions when adding/removing a polyfill branch (raising the floor is a major bump)
- `dhpk-legacy-characterization-tests` — for the single-version case where the polyfill branch IS the legacy code
