...
`$js-static-check-strategy status [--path <directory>]` owns the
`/dhpk:ts-check-status` status contract. With `--path`, resolve the directory
from the consumer repository's working directory; without it, use
`CLAUDE_PLUGIN_OPTION_JS_CHECK_PATH` or the `js/` default. Scan only immediate
regular `*.js` files, classify anchored `// @ts-check` and `// @ts-nocheck`
directives (with `nocheck` taking precedence), and report the deterministic
JSON summary `total=N strict=N nocheck=N unmarked=N` plus the file buckets.

A missing or unreadable scan path returns `status: UNAVAILABLE`, prints a
diagnostic, exits without fabricated counts, and is not a passing status. The
mode is read-only: it does not edit directives, configuration, or source
files. The package-local `scripts/status.js` runner accepts
`[--path <directory>]`, emits one JSON report on stdout, and exits `0` for a
completed `PASS` or `UNAVAILABLE` scan (argument/runner errors exit non-zero).
...
- `skills/dhpk-js-lint-config/SKILL.md` — companion skill with the
  ESLint tier model, custom AST selectors, and the legacy-globals
  three-list sync detail.

## When NOT to Use

Not for day-to-day lint-rule lookups (use the sibling `dhpk-js-lint-config` and
