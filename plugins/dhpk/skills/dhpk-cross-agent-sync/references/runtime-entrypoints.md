...
`dhpk-cross-agent-sync` is bundled at different paths in the supported harnesses.
...
```bash
SYNC_CLI="${CLAUDE_PLUGIN_ROOT}/skills/dhpk-cross-agent-sync/scripts/multi_ai_sync.py"
```
...
```bash
SYNC_CLI=".codex/skills/dhpk-cross-agent-sync/scripts/multi_ai_sync.py"
```
...
```bash
SYNC_CLI="skills/dhpk-cross-agent-sync/scripts/multi_ai_sync.py"
```
