...
| `codex-bridge` | **CODEX=on only** — outsource a self-contained clear-spec task, or a blind second opinion, to gpt-5.5 via the Codex CLI (`codex exec`); output isolated in the subagent, relayed verbatim; mode-qualified alias (read-only → `codex-reviewer`, workspace-write → `codex-worker`); `codex-reviewer` is internal-only in this rollout; see §Implementation dispatch | — |
