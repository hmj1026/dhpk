...
Resolve the harness directory with `dhpk-harness-revise` before collecting skills or rules so scans cannot mix platforms.
...
```bash
bash ~/.claude/skills/dhpk-rules-distill/scripts/scan-skills.sh
```

#### 1b. Collect rules index

```bash
bash ~/.claude/skills/dhpk-rules-distill/scripts/scan-rules.sh
```
