...
Resolve `HARNESS_DIR` using `dhpk-harness-revise` before counting harness components.
...
- Running `/dhpk:dhpk-harness-budget` command (this skill backs it)

## When NOT to Use

- Trimming, deduplicating, or validating harness structure — use `dhpk-harness-revise`.
- Backfilling missing `.claude/` skills / agents / rules — use `dhpk-harness-fill`.
- Auditing `.claude/` config health, naming, or hook wiring — use `/dhpk:dhpk-claude-health`.
...
**Verbose mode**
```
User: /dhpk:dhpk-harness-budget --verbose
