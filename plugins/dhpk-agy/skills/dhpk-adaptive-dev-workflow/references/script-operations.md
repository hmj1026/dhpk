...
> `${CLAUDE_PLUGIN_ROOT}/skills/dhpk-adaptive-dev-workflow/scripts/`。
> 若 `CLAUDE_PLUGIN_ROOT` 未設定（手動安裝），fall back 到
> `~/.claude/skills/dhpk-adaptive-dev-workflow/scripts/`。以下用 `$SCRIPTS` 代表該目錄。
...
```bash
SCRIPTS="${CLAUDE_PLUGIN_ROOT:-$HOME/.claude}/skills/dhpk-adaptive-dev-workflow/scripts"
