...
1. Run `bash "${CLAUDE_PLUGIN_ROOT}/scripts/run-skill.sh" dhpk-next-step analyze.js` to collect deterministic findings
...
Normal `dhpk-next-step` (no `--go`) is always advisory: it presents the
...
| State | Suggest |
|-------|---------|
| tech-spec exists, no request | `/dhpk:dhpk-create-request` |
| request exists, no tech-spec | `/dhpk:dhpk-tech-spec` |
| Both exist, code changed | `/update-docs` + `/dhpk:dhpk-create-request --update` |
