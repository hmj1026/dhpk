...
| Canonical skills | 100 個扁平 `dhpk-*` package | 每個 capability 只有一個具名 package，來源固定在 `skills/dhpk-*/`；內部 runtime package 不可呼叫，module 與 Codex 專案面只做 projection，不是第二份來源。 |
...
├── skills/                       # SSOT：100 個扁平 canonical skill，皆為 skills/dhpk-<name>/
