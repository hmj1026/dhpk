...
def cmd_validate(args):
    ...
    report = run_validation(
        ...
        agy_runtime_probe=args.agy_runtime_probe,
    )
    if args.format == "json":
...
def cmd_apply(args):
    ...
    repo_root = os.path.abspath(args.root)
    try:
        plan, digest = load_plan_with_digest(args.plan)
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        print("BLOCKED: unsafe plan: %s" % exc)
        return 2
    if not args.dry_run and args.approved_plan_sha256 != digest:
        print("BLOCKED: live apply requires --approved-plan-sha256 %s from the reviewed dry-run" % digest)
        return 2
    report = apply_plan(
        plan,
        repo_root,
    ...
    if args.update_tasks and not args.dry_run:
        try:
            tasks_path_abs = safe_repository_path(args.update_tasks, repo_root, ("openspec/changes",), must_exist=True)
        except ValueError as exc:
            print("BLOCKED: unsafe --update-tasks path: %s" % exc)
            return 2
        report["tasks_update"] = update_tasks_from_apply_report(tasks_path_abs, report)
    if args.manual_draft_output and not args.dry_run:
        try:
            draft_path_abs = safe_repository_path(args.manual_draft_output, repo_root, ("artifacts",))
        except ValueError as exc:
            print("BLOCKED: unsafe --manual-draft-output path: %s" % exc)
            return 2
        draft_md = render_manual_draft_markdown(report)
        atomic_write_text(draft_path_abs, draft_md)
        report["manual_draft"] = {
...
def build_parser():
    ...
    p_validate.add_argument(
        "--agy-runtime-probe", action="store_true",
        help="執行有界、唯讀的 agy Subagent smoke probe；未指定時只記錄 NOT_RUN",
    )
    ...
    p_apply.add_argument("--approved-plan-sha256", default="", help="live apply 必須綁定已審核 dry-run 的 plan SHA-256")
