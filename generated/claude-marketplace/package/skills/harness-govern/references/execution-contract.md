...
python3 -B "$SYNC_CLI" plan --format json \
  --output /tmp/harness-govern-sync-plan.json
```
...
python3 -B "$SYNC_CLI" openspec-tasks \
  --plan /tmp/harness-govern-sync-plan.json \
...
python3 -B "$SYNC_CLI" apply \
  ...
  --dry-run \
  --format markdown \
  --output /tmp/harness-govern-sync-apply-dryrun.md
```
...
python3 -B "$SYNC_CLI" apply \
  --plan /tmp/harness-govern-sync-plan.json \
  --approved-plan-sha256 <sha256-from-dry-run> \
  --format markdown \
  --update-tasks openspec/changes/claude-sync-YYYY-MM-DD/tasks.md \
  --manual-draft-output artifacts/harness-govern-sync-manual-draft-YYYY-MM-DD.md \
  --output artifacts/harness-govern-sync-apply-YYYY-MM-DD.md
```
