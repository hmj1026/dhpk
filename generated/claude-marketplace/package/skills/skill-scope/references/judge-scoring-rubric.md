...
Self-contained scoring reference for `skill-scope`'s `judge` mode. Load this when scoring a skill across the 8 dimensions, assigning points, or computing the grade. Each dimension lists score bands, green/red flags, examples, and the evaluation test to apply.
