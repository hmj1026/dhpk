...
- `ruff check` (lint) + `ruff format` (formatting) — single tool, fast. ccas selects
  rule families `E, F, I, N, W, UP`. See the `dhpk-python-static-checks` skill for the
  selection strategy.
...
For ruff/pyright config see `dhpk-python-static-checks`; for web-API specifics see
`dhpk-fastapi-pro`; for tests see `dhpk-pytest-async`.
