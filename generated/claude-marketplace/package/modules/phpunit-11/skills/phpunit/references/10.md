...
> PHPUnit 9 until you can bump PHP. See the PHPUnit 9 family reference
> (`references/9.md`) for the latest version that works on 7.3+.

---

## Attribute-style annotations (the headline change)

Doc-comment annotations still work in 10 and remain compatible for existing tests. Attributes are preferred for new tests; PHPUnit 11 deprecates ordinary comment metadata and PHPUnit 12 removes it except the @codeCoverageIgnore* family.
...
### After (PHPUnit 10 preferred; migrate before PHPUnit 12)
...
- [ ] Optional: start migrating ordinary test metadata → attributes (recommended in 11; required for ordinary metadata in 12)

---

## When NOT to Use

- PHP 7.x / 8.0 projects — PHPUnit 10 won't install; use references/9.md.
- A PHPUnit 11 project — doc-comment annotations are deprecated there; use
  the PHPUnit 11 family reference (`references/11.md`).
...
Lifecycle claims are based on the official PHPUnit 10.5 annotations manual (https://docs.phpunit.de/en/10.5/annotations.html), PHPUnit 11.5 annotations manual (https://docs.phpunit.de/en/11.5/annotations.html), and PHPUnit 12.5 code-coverage manual (https://docs.phpunit.de/en/12.5/code-coverage.html), checked 2026-09-02. PHPUnit 10 checks attributes before comments and still reads comment metadata; ordinary comment metadata is deprecated in 11 and removed in 12, while @codeCoverageIgnore, @codeCoverageIgnoreStart, and @codeCoverageIgnoreEnd remain supported.
