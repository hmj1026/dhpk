...
`$SKILL_DIR` denotes the physical directory containing this `SKILL.md`;
`$PROJECT_DIR` denotes the consumer project root. These are resolved path
notations, not ambient environment-variable requirements. Run the local runner
from the consumer project after the existing execution approval boundary.
...
intended release file explicitly, unless the project ships
`scripts/release/prepare-release.js` with a `paths` subcommand that prints its release
scope (`file`/`dir`/`deleted` lines). With no file list, the runner then stages only
...
```bash
node "$PROJECT_DIR/scripts/release/publish-gate.js" --version "<version>"
```

The runner uses `$PROJECT_DIR/scripts/release/publish-gate.js` when present and refuses the
