# Skill Judge — Evaluation Philosophy

Self-contained mental model for `skill-scope`'s `judge` mode. Load this when you need the "why" behind the rubric — what a Skill fundamentally is, the knowledge-delta lens for the First Pass scan, and the meta-question to return to when a score feels off.
...
This skill-scope family (judge mode) should itself pass evaluation:
...
- **Progressive Disclosure**: Lean SKILL.md routing to references on demand
