...
Self-talk stop-loss reference for the rationalization phrasings that precede skipping a mandatory reviewer or TDD step. Not always-on; referenced from `${POLICY_BUNDLE_ROOT}/rules/execution-policy.md` "Anti-rationalization" section.

> **Load when**: feeling hesitant about skipping a reviewer or TDD obligation; self-reference before acting.
...
|---|---|
| "This change is small, no reviewer needed" | Review Gate obligations are **derived from the cumulative risk of every change**, not only large changes. Three-line changes can still trip CSRF / SQL injection / silent truncation traps. |
| "The runtime will catch it, just commit first" | Durable Review Gate evidence is a completion contract, not a reason to skip the reviewer. If a required lane has not returned a parseable result, the obligation remains unresolved. |
...
| Wanting to skip any required reviewer obligation | Load this file → consult the table for the counter-argument; if no counter is available, the reviewer MUST run |
| Wanting to invoke the append-only exemption (Glossary, `${POLICY_BUNDLE_ROOT}/rules/execution-policy.md`) | Load row 4 of the table → enforce the "100% pure addition" check |
...
| About to commit or deploy a non-trivial / irreversible decision (branching logic, boundary crossing, migration, public API change) | Load this file → run the in-flight doubt cycle (`${POLICY_BUNDLE_ROOT}/rules/execution-policy.md`) before it stands |
...
- `${POLICY_BUNDLE_ROOT}/rules/execution-policy.md` — reviewer dispatch + Self-check SSOT
