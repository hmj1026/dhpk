...
|---|---|---|
| `^5.6` or `^7.0` | param class type hints, scalar type hints (7.0+) | **return-type declarations**, `??`, arrow fns, named args, union types, group `use`, multi-catch, short list `[$a,$b]=`. Follow the project's PHP coding rules. |
| `^7.1+` (incl. `^7.3`, `^8.0`+) | return-type declarations, `??`, nullable types, `void` return, `?:`; `mixed` (8.0+); native enums (8.1+); union types (8.0+); `match` (8.0+); readonly (8.1+) | anything above the project's stated floor; match an explicit convention in AGENTS.md / openspec config if present |
