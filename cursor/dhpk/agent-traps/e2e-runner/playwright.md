...
|---|---|---|
| `boundingBox()` has no `.top`: it returns `{x, y, width, height}` (unlike DOM `getBoundingClientRect()`). `boundingBox()?.top` is always `undefined`, and a `?? 0` fallback silently masks this — you read `0` and never notice. | Use `.y` for the top offset. | `boundingBox().y` already used |
