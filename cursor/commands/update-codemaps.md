...
rewriting application source. See the [command contract](https://github.com/hmj1026/dhpk/blob/main/docs/agent-guidance/command-contract.md).
...
> Proactive orchestration: after structural changes (new module / renamed dir / new public service) the `doc-updater` agent runs this command (and `/update-docs`) — see the back-stop in `.cursor/dhpk/policies/execution-policy.md`. Invoke `/update-codemaps` directly for a one-off manual refresh.
