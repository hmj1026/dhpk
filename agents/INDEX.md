...
| [codex-bridge](codex-bridge.md) | sonnet | **CODEX=on only** — thin bridge that outsources a self-contained clear-spec task, or a blind second opinion, to the GPT-5.6 family via the Codex CLI (`codex exec`); read-only resolves to `gpt-5.6-sol`/`high`, workspace-write to `gpt-5.6-luna`/`xhigh`; uses an immutable dispatcher-attested transport context while retaining the three-argument wrapper shape, and relays Codex's output **verbatim** (output isolated in the subagent) |
