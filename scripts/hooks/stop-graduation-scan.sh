...
# - OpsX draft (optional): when an openspec/changes/ dir exists, auto-drafts a
#   change skeleton for high-confidence entries. Guarded — skipped entirely in
#   non-OpsX projects and under CLAUDE_HOOK_SKIP_OPSX_DRAFT=1.
...
# === Test-mode isolation ===
# CLAUDE_HOOK_TEST_MODE=1 redirects all state into $CLAUDE_HOOK_TEST_OUTDIR and
# sets CLAUDE_HOOK_SKIP_OPSX_DRAFT=1 so smoke tests never write into a real
...
# Extract transcript_path (multiple field names across Claude Code versions).
...
# === Optional: graduation → OpsX change draft ===
# For entries with confidence ≥ 0.7, count ≥ 3, passing the time-span gate, and
# whose source memory file still exists, auto-draft an OpsX change skeleton when
# the project has an openspec/changes/ dir. Advisory only — never git-added.
...
> Auto-drafted by the dhpk stop-graduation-scan hook on {now_iso}.
> Source memory entry: `{memory_path}`
> Graduation signal: count={rec['count']}, confidence={rec['confidence']:.2f}
>
> **Status: DRAFT** — generated by a hook, unreviewed. Run
> `/opsx:continue {slug}` to flesh out Why / What Changes / Capabilities, or
> delete this directory to reject (counts.json keeps accruing for next time).

## Why

`{entry}` has been cited across {rec['count']} sessions at confidence
{rec['confidence']:.2f} ≥ 0.7 (no trap re-occurrence signal) — repeatedly
validated as worth remembering, meeting graduation criteria (promote from
ad-hoc memory to a `.claude/rules/` entry or skill).

Fill in:
- what problem class this entry covers (trap / convention / workflow / domain)
- why it should graduate now rather than stay in memory (signal vs noise)
- relationship to existing rules / skills (reinforce / supersede / complement)

## What Changes

- target location (`.claude/rules/<path>.md` or `.claude/skills/<name>/SKILL.md`)
- whether to remove the original entry from `MEMORY.md` (typically: yes, keep a
  pointer to the new location)
- whether `graduation-candidates.md` needs updating (typically: the hook syncs
  it on the next run)

## Capabilities

Add the matching capability slug, or declare this graduation has none (pure
internal harness maintenance).

## Out of scope

- rewriting the source entry's content (copy wording verbatim)
- bundling other entries into this proposal (one graduation per change)
"""
        tasks_md = f"""# Tasks — {slug}

## 1. Confirm graduation worthiness

- [ ] 1.1 Read `{memory_path}` in full
- [ ] 1.2 Compare against existing `.claude/rules/**` and dhpk plugin rules — no dupes
- [ ] 1.3 Decide landing spot: local rule / local skill / dhpk PR

## 2. Migrate content

- [ ] 2.1 Write the content at the target location (preserve original wording)
- [ ] 2.2 Replace the `MEMORY.md` entry with a pointer to the new location
- [ ] 2.3 Update cross-references in other memory entries (grep `{entry}`)

## 3. Verify

- [ ] 3.1 Run `dhpk:claude-health` to confirm .claude/ structure is intact
- [ ] 3.2 Run `dhpk:harness-revise` to verify no triggers were lost
- [ ] 3.3 If graduating into dhpk, open a dhpk PR and link this change
"""
        try:
