...
#
# Adding/removing a slot means editing scripts/lib/sentinel-slots.json and running
# `node scripts/ci/gen-slots.js --write`. Hooks that iterate SENTINEL_NAMES
# (clear-sentinel, reap-stale-sentinels, pre-bash-guard push-block,
# stop-review-reminder) extend automatically as long as the generated arrays stay
# aligned. SENTINEL_SHORT_NAMES carries compact per-slot labels for space-constrained
# consumers (statuslines etc.).
...
# Review sentinel slots: 0=code 1=db 2=sec 3=frontend 4=doc 5=polyfill 6=migration
...
if [ -n "$_dhpk_review_agents" ]; then
    IFS=',' read -r -a SENTINEL_AGENTS <<< "$_dhpk_review_agents"
else
