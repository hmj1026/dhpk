...
function validateReceipt(receipt, roots) {
  ...
  for (const field of ['planFingerprint', 'artifactFingerprint']) {
    if (!DIGEST_PATTERN.test(receipt[field] || '')) {
  ...
  for (const field of ['profileFingerprint', 'selectionFingerprint', 'surfaceSelectionFingerprint', 'capabilityEvidenceFingerprint']) {
    if (receipt[field] !== null && receipt[field] !== undefined && !DIGEST_PATTERN.test(receipt[field])) {
      throw fail('INVALID_RECEIPT', `project projection receipt field '${field}' is invalid`);
    }
  }
  ...
  for (const entry of receipt.entries) {
    ...
    if (receipt.sourceFingerprints[entry.stableId] !== entry.sourceFingerprint) {
...
function adoptLegacyManifest(roots) {
  ...
  const managedPaths = manifest.managedPaths.slice().sort();
  for (const relative of managedPaths) {
    ...
    if (!stat || stat.isSymbolicLink() || !stat.isFile() || digest(fs.readFileSync(target)) !== manifest.generatedFingerprints[relative]) {
  ...
  const adopted = sealReceipt({
    ...
    entries,
    managedPaths,
    generatedFingerprints: Object.fromEntries(managedPaths.map((relative) => [relative, manifest.generatedFingerprints[relative]])),
    sourceFingerprints: {},
    rollback: null,
    legacyUnbound: true,
  });
...
function validateManagedFiles(roots, receipt) {
...
function publishManagedCandidate({ roots, previous, candidateRoot, candidateFiles, candidateFingerprints, nextReceipt, createRollback = true }) {
  ...
  try {
    ...
    fs.rmSync(journalPath, { force: true });
    return { receipt, published: true, managedPaths: candidateFiles.slice().sort() };
  } catch (error) {
    if (!published) {
      try {
...
function receiptForArtifact({ roots, plan, staged }) {
  ...
  return sealReceipt({
    ...
    hostBindings: clone(plan.hostBindings || {}),
    bindings: clone(plan.hostBindings || {}),
...
function validateRelocatableAgentsSkillsProjection(options = {}) {
  ...
  try {
    ...
    if (options.inventory && (options.sourceRoot || options.root)) {
      const plan = compilePlan({ inventory: options.inventory, plan: options.plan, profileId: options.profileId || receipt.profileId || 'portable-core', requestedHosts: options.requestedHosts });
