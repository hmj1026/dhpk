...
function resolveSafeSkillPath(root, entry) {
  const relative = entry && (entry.path || path.join('skills', entry.id || ''));
...
function skillPackageManifestPath(root, entry) {
  const skillPath = resolveSafeSkillPath(root, entry);
  if (skillPath.error) return skillPath;
  return { path: path.join(skillPath.path, 'skill-package.json') };
}

function readSkillPackageManifest(root, skillId) {
  const result = skillPackageManifestPath(root, { id: skillId, path: path.join('skills', skillId) });
...
function resolveSkillPackageClosure(root, selectedEntries, { availableEntries = selectedEntries, surface = null } = {}) {
  ...
  const visit = (entry, chain) => {
    ...
    const manifestResult = skillPackageManifestPath(root, entry);
    if (manifestResult.error) throw new Error(manifestResult.error);
    const manifestPath = manifestResult.path;
    if (fs.existsSync(manifestPath)) {
      ...
      for (const dependency of manifest.requires || []) {
        ...
        const dependencyManifestResult = skillPackageManifestPath(root, dependencyEntry);
        if (dependencyManifestResult.error) throw new Error(dependencyManifestResult.error);
        const dependencyManifestPath = dependencyManifestResult.path;
