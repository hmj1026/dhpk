...
This remains additive to the existing inventory schema during migration. The bootstrap/reconciliation command preserves explicit projection policy rather than regenerating it from directories. The closed symlink policy vocabulary is `forbid`, `contained-relative`, and `declared-source-relative`. `codex-sync` may use only `declared-source-relative`: the destination link must be owned by the plan and its relative target must resolve inside the plan-bound canonical source root. Existing absolute-link behavior is an observable compatibility constraint and requires characterization before any conversion to relative links.
...
`EvidenceResult.verdict` remains the closed seven-state vocabulary already used by the repository. Lifecycle presentation codes such as `INSTALL_PASS` and `CONSUMER_BLOCKED` are not evidence verdicts and must be carried by a separate lifecycle aggregate; they must never be passed to Sentinel clearance.
