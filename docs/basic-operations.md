...
| `--turns N` | Overrides the auto-calculated turn budget (default: `max(20, min(120, open_tasks × 4 + 20))`). |
...
**What the pasted `/goal` string actually contains:** the kickoff instruction to invoke `opsx:apply` on the change; a condition that every `tasks.md` checkbox is `[x]`; a universal check that no reviewer sentinel is pending (`ls .claude/artifacts/sessions/.pending-*` outputs `NONE`); one line per verification gate actually detected for this change (test runner, build, lint, coverage, smoke — only what applies, nothing forced); and stop limits (the turn budget, the optional wall-clock limit, and a hard-rule-escalation clause that halts and writes a resume note rather than guessing through an explicit policy conflict). Full structure: `skills/opsx-apply-goal/SKILL.md` Step 6 (Parts 0-4).
