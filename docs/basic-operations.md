...
**What the pasted `/goal` string actually contains:** an orientation-first kickoff — run one Bash orientation command (`head -40 openspec/changes/<change-id>/tasks.md`) first, *then* invoke `opsx:apply` on the change, with a fallback if the skill catalog hasn't registered yet (`Unknown skill` → retry once next turn → still failing → implement directly from the change directory's `proposal.md`/`design.md`/`tasks.md`); a self-locating pointer to `rules/execution-policy.md` (`$CLAUDE_PLUGIN_ROOT` first, else the newest installed plugin cache path, else the goal string's own gates — never a filesystem scan) — emitted regardless of the `orchestration_dispatch` setting, since only the dispatch directive itself is gated by it; a condition that every `tasks.md` checkbox is `[x]`; a universal check that no reviewer sentinel is pending (`ls .claude/artifacts/sessions/.pending-*` outputs `NONE`); one line per verification gate actually detected for this change (test runner, build, lint, coverage, smoke — only what applies, nothing forced); and stop limits (the turn budget as a hard checkpoint — write `.resume-note.md` and end the session, not advisory; the optional wall-clock limit; a blocked-on-human clause that annotates `[blocked: <reason>]` in `tasks.md` and stops when every remaining task needs a human-only action; and a hard-rule-escalation clause that halts and writes a resume note rather than guessing through an explicit policy conflict). Full structure: `skills/opsx-apply-goal/SKILL.md` Step 6 (Parts 0-4).

When `orchestration_dispatch=on` (default), the generated `/goal` condition also embeds a one-line dispatch directive routing implementation through `deep-reasoner` / `fast-worker` (see item 9 below). Set `orchestration_dispatch=off` to fully opt out of that dispatch directive — implementation stays inline everywhere instead of being routed through worker agents.

**4000-character paste limit:** Claude Code's `/goal` input has a practical paste ceiling around 4000 characters. If the composed goal string would exceed it, `opsx-apply-goal` first auto-compacts Part 0 — the kickoff sentence, the dispatch directive (when `orchestration_dispatch=on`), and the CODEX statement (when `--codex` is set) — preserving the same rules with shorter wording; if it's still over the limit after compacting, no `/goal` command is emitted at all — instead you get the character count and which setting or flag to adjust (turn off the `orchestration_dispatch` project setting, or drop `--codex` / `--smoke`) before re-running.
