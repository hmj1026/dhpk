...
**貼上的 `/goal` 字串實際內容：** 以「先定位再行動」開場——先執行**單一組合的** Bash 定位指令，同時讀取該變更的任務標頭（`head -40 openspec/changes/<change-id>/tasks.md`）與自我定位的 `rules/execution-policy.md`（優先 `$CLAUDE_PLUGIN_ROOT`，其次最新安裝的 plugin cache 路徑，再其次得到 `POLICY-UNRESOLVED`——此時退回 goal 字串自身的內嵌條件；此讀取為 best-effort），*再*呼叫 `opsx:apply`；若此時 skill 目錄還沒註冊好，附一條備援（`Unknown skill` → 下一回合重試一次 → 仍失敗 → 直接讀該變更目錄的 `proposal.md`/`design.md`/`tasks.md` 並直接實作）。Part 0 本身現在是一段有界的開場：開場句、hard-rule 例外條款、Unknown-skill 備援、一行「四角色」派發名單（`dhpk:fast-worker` / `dhpk:deep-reasoner` / `dhpk:tdd-guide` / `dhpk:e2e-runner`——絕不用 `general-purpose`——僅在整個實作步驟的足跡 ≤2 個檔案加上記帳工作時才內聯處理）、內嵌的 hard-rule 防護、以及一行 CODEX ON/OFF 宣告；行為面的細節（premise-verification 路由、in-flight doubt cycle、CODEX 高風險 peer 路徑、session 結束自我檢查）不再放在 goal 字串裡——現在只存在於 `rules/execution-policy.md`，並透過定位讀取來綁定。其餘條件維持不變：「`tasks.md` 所有 checkbox 都是 `[x]`」的條件；「沒有待處理的 reviewer sentinel」的通用檢查（`ls .claude/artifacts/sessions/.pending-*` 輸出 `NONE`）；針對這次變更**實際偵測到**的每個驗證門檻各一行（測試 runner、build、lint、覆蓋率、smoke——只列出適用的，不會強加）；以及停止條件（回合預算作為硬性檢查點——寫入 `.resume-note.md` 並結束 session，而非僅供參考；可選的時鐘上限；當剩餘任務全部卡在只有人類才能做的動作時，在 `tasks.md` 標註 `[blocked: <reason>]` 並停止的 blocked-on-human 條款；以及遇到明確政策衝突時停下並寫入 resume note 的 hard-rule-escalation 條款，而非硬猜）。完整結構見 `skills/opsx-apply-goal/SKILL.md` Step 6（Part 0-4）。

當 `orchestration_dispatch=on`（預設）時，產生的 `/goal` 條件也會內嵌一行 dispatch 指示，透過四角色名單——`fast-worker`、`deep-reasoner`、`tdd-guide`、`e2e-runner`——路由實作工作（見下方第 9 項）。設為 `orchestration_dispatch=off` 可完整關閉該 dispatch 指示——實作工作一律內嵌執行，不再透過 worker agent 路由。

**4000 字元硬性停止：** Claude Code 的 `/goal` 輸入有大約 4000 字元的實際貼上上限。現在只有單一完整範本——沒有精簡版備援。一般變更組成的字串長度約 3300 字元。若組成的 goal 字串會超過 4000 字元，這被視為「理論上不該發生」的範本退化，而不是常規情況：完全不會印出 `/goal` 指令——而是顯示實際量測到的字元數與該調整哪個設定或旗標（關閉 `orchestration_dispatch` 這個專案設定、或拿掉 `--codex` / `--smoke`）後再重新執行。
