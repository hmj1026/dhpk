...
完整旗標集：`--turns N`（覆寫自動計算的回合預算）、`--max-duration <Nm|Nh>`（時鐘停止條件）、`--min-coverage N`（強制套用覆蓋率門檻，即使專案本身沒有原生的覆蓋率設定）、`--dry-run`（印出 goal 字串，不需貼到新 session 即可預覽）。

當 `orchestration_dispatch=on`（預設）時，產生的 `/goal` 條件也會內嵌一行 dispatch 指示，透過 `deep-reasoner` / `fast-worker` 路由實作工作（見下方第 9 項）。設為 `orchestration_dispatch=off` 可取得與 v0.22.0 之前逐位元組相同的輸出。
...
兩者都需要 `mcp__codex__codex` / `mcp__codex__codex-reply` 工具，這來自 OpenAI 發布的**另一個獨立** Claude Code plugin，並非 dhpk 提供。安裝步驟與 `CODEX=on` 的 opt-in 機制（`/dhpk:do` 的 `--codex` flag／自然語言觸發）見 [`docs/configuration.zh-TW.md`](./configuration.zh-TW.md#codex-mcp-依賴並非-userconfig-旋鈕)。
