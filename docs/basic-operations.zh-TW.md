...
| `--turns N` | 覆寫自動計算的回合預算（預設：`max(20, min(120, 未完成任務數 × 4 + 20))`）。 |
...
**貼上的 `/goal` 字串實際內容：** 啟動 `opsx:apply` 的指示；「`tasks.md` 所有 checkbox 都是 `[x]`」的條件；「沒有待處理的 reviewer sentinel」的通用檢查（`ls .claude/artifacts/sessions/.pending-*` 輸出 `NONE`）；針對這次變更**實際偵測到**的每個驗證門檻各一行（測試 runner、build、lint、覆蓋率、smoke——只列出適用的，不會強加）；以及停止條件（回合預算、可選的時鐘上限，以及遇到明確政策衝突時停下並寫入 resume note 的 hard-rule-escalation 條款，而非硬猜）。完整結構見 `skills/opsx-apply-goal/SKILL.md` Step 6（Part 0-4）。
