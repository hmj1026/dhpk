...
**`--fast-worker=<claude|codex|agy|auto>` 旗標：** 僅為這次呼叫選擇機械 worker，不改專案設定。`/dhpk:do` 在路由匹配前解析並移除此旗標，再把原值以 `FAST_WORKER_OVERRIDE` 只傳給實作型路由（`adaptive-dev-workflow`、`bug-fix`、`feature-dev`、`opsx-apply-goal`）。優先序為旗標 > `fast_worker_backend` userConfig > shipped `claude`；無效旗標警告一次後退回 userConfig／預設。下游流程一律呼叫共用 selector，不自行重做可用性、順序或 fallback 邏輯。
...
在 `feature-dev`、`bug-fix`、`adaptive-dev-workflow`、`opsx-apply-goal` 的實作階段，推理密集工作交給 `deep-reasoner`；機械式工作經共用 selector 交給 `fast-worker`、`codex-fast-worker` 或 `agy-fast-worker`。`auto` 依 `fast_worker_backend_order`；只有選定 CLI 執行檔缺失時可 fallback 到 Claude，auth、model、execution、task 或 verification 失敗都不得 fallback。`--fast-worker=codex` 與 `CODEX=on` 無關：前者選 Codex CLI 機械 worker，後者啟用 Codex MCP peer 路徑。整個實作步驟合計 ≤2 個檔案的小 diff 可內聯；dispatch 開啟時禁止 `general-purpose`。完整分派表見 [`rules/execution-policy.md`](../rules/execution-policy.md) §"Implementation dispatch"。
