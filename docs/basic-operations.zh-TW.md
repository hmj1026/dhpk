...
**4000 字元硬性停止：** Claude Code 的 `/goal` 輸入有大約 4000 字元的實際貼上上限。現在只有單一完整範本——沒有精簡版備援。一般變更組成的字串長度約 3600 字元。若組成的 goal 字串會超過 4000 字元，這被視為「理論上不該發生」的範本退化，而不是常規情況：完全不會印出 `/goal` 指令——而是顯示實際量測到的字元數與該調整哪個設定或旗標（關閉 `orchestration_dispatch` 這個專案設定、或拿掉 `--codex` / `--smoke`）後再重新執行。
