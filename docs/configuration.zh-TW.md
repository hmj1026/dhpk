...
dhpk 在 `.claude-plugin/plugin.json` 中暴露 **40 個 `userConfig` 旋鈕**。本頁完整記錄每個旋鈕：在哪裡設定、可接受哪些值、實際會改變什麼。日常操作流程（安裝、常見工作流、review 循環）請見 [`docs/basic-operations.zh-TW.md`](./basic-operations.zh-TW.md)。
...
`orchestration_dispatch` 的 `CODEX=on` peer 路徑，以及 6 個 `codex-*` skill（`codex-architect`、`codex-brainstorm`、`codex-cli-review`、`codex-code-review`、`codex-explain`、`codex-implement`），都需要 `mcp__codex__codex` / `mcp__codex__codex-reply` 工具。dhpk 本身**不**內附或設定這些工具——它們來自 OpenAI 發布的**另一個獨立** Claude Code plugin，而非任何 dhpk 的 `userConfig` key。
...
`/codex:setup` 會回報 Codex 是否就緒，若偵測到 npm，會提議幫你執行 `npm install -g @openai/codex`。前置需求：Node.js 18.18 以上，以及 ChatGPT 訂閱（含 Free 方案）或 OpenAI API key 擇一。若 Codex 已安裝但尚未登入，執行 `codex login`（在 Claude Code session 內可加 `!` 前綴當作 shell 指令執行）。

**驗證 MCP 連線**：安裝 `openai/codex-plugin-cc` 只是向 Claude Code 註冊了一個 Codex MCP server 項目；真正把它啟動並建立連線的是 `/reload-plugins`（或開一個新 session）——單純安裝並不會啟動連線。要確認連線是否成立：
...
1. 重新執行 `/reload-plugins`，再檢查一次 `/mcp`——連線是在 plugin 載入時建立，不是安裝當下。
2. 若仍然缺席，執行 `/codex:setup`——它會重新檢查底層 `codex` CLI 的安裝與登入狀態，MCP server 的啟動依賴這些條件。
3. 若 `codex` 已顯示連線，但某個 `codex-*` skill 仍然失敗，問題通常出在登入驗證（`codex login`），而不是 MCP 連線本身。

未安裝此 plugin 時，呼叫任一 `codex-*` skill 會出現工具權限錯誤（找不到 `mcp__codex__*`）——dhpk 對這幾個 skill 沒有降級路徑，因為它們的存在目的就是委派給 Codex。這與 `CODEX=on`（見 [`docs/basic-operations.zh-TW.md`](./basic-operations.zh-TW.md#9-implementation-dispatch自動)）是兩回事——後者是**單次 session 的 opt-in flag**，不是持久化的 `userConfig` 值，沒有安裝時 `--config` 的對應項，且每個 session 都會重置，除非再次帶 `--codex` 或說「用 codex」。

這也與 **Codex CLI 雙軌同步**（`install-codex-skills.sh`，見 [`docs/basic-operations.zh-TW.md`](./basic-operations.zh-TW.md)）無關——那是把 dhpk 自己的 skill 鏡射進專案的 `.codex/` 目錄，給直接執行獨立 `codex` CLI 工具的人使用，那條路徑完全不需要 MCP server。

**MCP 與 Skill 兩種介接方式，以及一個容易混淆的命名撞名**：`openai/codex-plugin-cc` 同時提供兩種不同的介接介面，dhpk 只用了其中一種。

- **MCP**（上述 6 個 `codex-*` skill 實際呼叫的方式）：`mcp__codex__codex` / `mcp__codex__codex-reply` 是透過 Model Context Protocol 的型別化工具呼叫——與常駐的 Codex MCP server process 建立持久連線。dhpk 自己的 skill 檔案會帶入 `prompt`、`sandbox`、`approval-policy` 等結構化參數（範例可見 `skills/codex-architect/SKILL.md` 或 `skills/codex-code-review/references/codex-prompt-fast.md`），並收到結構化回傳結果。實際參數 schema 由上游 plugin 擁有，可能獨立於 dhpk 演進——上述參數是 dhpk 目前隨附的 skill 實際依賴的參數。
- **Skill**（dhpk 沒有任何 skill 使用這個方式）：同一個 plugin 也提供 `/codex:review`、`/codex:setup` 這類 slash-command skill。這些是透過 Claude Code 的 Skill 機制載入 model context 的 Markdown 指令——沒有型別化 schema、沒有持久連線，由該 skill 自行決定如何呼叫 Codex CLI，輸出也是自由格式文字。

請不要把上述兩者與 dhpk **自己的** `.codex-plugin/` 目錄搞混（見 [`docs/basic-operations.zh-TW.md`](./basic-operations.zh-TW.md) 的「Codex Plugin Marketplace」章節）——那份 manifest 是反方向的：讓 **Codex CLI** 把 dhpk 的 skill 當作 Codex 原生 plugin 安裝。它只是恰好也用了「plugin」這個字，跟 `openai/codex-plugin-cc` 或上面說的 MCP 工具完全無關。
