...
| `orchestration_dispatch` | string | `on` | `on` \| `off` | Implementation dispatch 分派表（`deep-reasoner` / `fast-worker` 在 `adaptive-dev-workflow` 的 feature/bug mode 與 `opsx-apply-goal` 中的路由）的關閉開關。`on` 時實作階段工作依決策表路由，並禁止用 `general-purpose` 執行實作。`off` 完整還原 v0.22.0 之前的行為：內聯實作、不禁止派發、`opsx-apply-goal` 輸出與舊版逐位元組相同。 |
